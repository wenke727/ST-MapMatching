...
def combine_links(edges, combine_intervals):
    ...
    """
    edges.loc[:, 'waypoints'] = edges.apply(lambda x: f"{int(x.src)},{int(x.dst)}", axis=1)
    ...
    for start, end, _ in combine_intervals:
        ...
        nids = np.append(segs.src.values, segs.iloc[-1]['dst'])

        edges.loc[start, 'dst'] = segs.iloc[-1]['dst']
        edges.loc[start, 'dist'] = segs.dist.sum()
        # FIXME maybe dulplicate
...
def pipeline_combine_links(df_edges:pd.DataFrame, exclude_list, n_jobs=8):
    ...
    aux_edge_intervals = cands_edges.sort_values(by=["way_id", 'order'])\
                                    .groupby('way_id')\
                                    .order.apply(list)\
                                    .apply(lambda lst: merge_intervals([[i-1, i] for i in lst if i > 0]))

    _df_edges = df_edges.query(f"way_id in @cands_way_ids")
    params = ((df, aux_edge_intervals[i]) for i, df in _df_edges.groupby('way_id'))
    ...
    keep_edges.loc[:, 'waypoints'] = keep_edges.apply(lambda x: f"{int(x.src)}, {int(x.dst)}", axis=1)
